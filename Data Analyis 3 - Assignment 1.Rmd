---
title: "Data Analyis 3 - Assignment 1"
author: "Rauhan Nazir"
date: "1/25/2022"
output: pdf_document
---
...
summary_table <- datasummary(wph + grade92 + age ~ Mean + SD + Min + Max + P25 + P75 + N , data = elementary_middle_school_teachers)
...
Avgrmse <- ggplot( m_comp , aes( x = complexity , y = RMSE ) ) +
...
# plotting results
pr <- ggplot(elementary_middle_school_teachers, aes(x=predict(reg2, elementary_middle_school_teachers), y=wph)) + 
  geom_point(alpha = 0.5) +
  geom_abline(intercept = 0, slope = 1, size = 0.5) +
  scale_x_continuous(limits = c(0,50)) + 
  scale_y_continuous(limits = c(0,80)) +
  theme_bw() +
  labs(x= "Predicted Values - Wage per Hour USD", y= " Actual Values - Wage per Hour (USD)")
...
The goal of this assignment is to build four predictive models using linear regression for **earnings per hour**. I started with building a simple model and with every model increased it's complexity by adding variables including squared terms and interaction variables. After building these models, I moved towards evaluating them on the basis of RMSE in the full sample, cross-validated RMSE and BIC in the full sample. The data used was extracted from **[cps-earning dataset]('https://osf.io/g8p9j/')**. The occupation that I decided to go with was **Elementary and Middle School Teachers** (Occupation code:2310) and the rationale behind that will be explained further in the report. 


## Data Cleaninng & Munging 

To begin with, I counted the number of observations under each occupation and decided to go with the one with the highest number of observations, as a bigger sample size would help in building relatively accurate predictive models.
The next step was to create the target variable, **earnings per hour**(weekly earnings/number of hours) as it was not present in the data set. With the required data set loaded and target variable created, I went through rigorous exploratory analysis and data cleaning to ensure that the data being inputted into the regression models did not contain any unwanted or irrelevant observations and was actually a result of conscious decisions taken(garbage in garbage out). 

I only wanted to focus on the full time teachers, for that I excluded all the teachers who were working less than 32 hours per week (As per the information on IRS website). The mean of wage per hour of all the teachers is around **USD 26**, keeping that in mind, I decided to exclude the ones with a wage per hour of less than **USD 10** as it could have been a result of discrepancies in the data. Another important decision that I had to take was on how I was going to deal with the NAs. The only variable with NA values was teachers ethnicity, that too in large number (2970). With over 90% of NA values, it would not make much sense to include ethnicity in the model. Majority of the teachers are of Grade 43 and above (Over 95%), so I made the decision of dropping the teachers with lower level of qualifications. 
The next step was creating factors for variables that were going to be included in the regressions. I created factor variables for grade92, martial, race and sex stored into variables called **educ**, **married_status**, **race** & **gender**. I also modified the **ownnchild** variable as well, where I converted it into a binary, as I was only interested in whether having children had an impact on wages, not the number of children. There did not seem to be much difference within government and private classes so I clubbed them together, as just being as Government and Private. Same was done for **prcitshp** and store into a variable called **origin**, with either a teacher being a native or a foreigner. 

## Interaction Terms and Regressions 
The first step in this process was building and exploring the interaction terms. The main rationale behind this was to tackle the issue of multicollinearity. If the predictive variables were related with each other, and we included them in our regression models, it would result in inaccurate results. However, to build an exhaustive list of all the interaction terms is not feasible as it would explode the number of variables used in the regressions. I decided to explore and include the ones which I thought made most sense. There were a total of 11 interaction terms that I explored and all of these were included in **Model 4**(the most complex one in terms of number of variables). The details of all the models created is as following:
**Model 1**:(wph ~ educ) The most simplest model where, wage per hour was only regressed on the qualification.
**Model 2**:(wph ~ educ + gender + age + agesq + region) Added some key variables without interaction terms.
**Model 3** & **Model 4** Included interaction terms, with Model 4 containing all of them.

## Interpreting the Results and Final Model Selection

The models were evaluated on 3 metrics (RMSE,BIC,K-fold RMSE). Overall **Model 2** performed the best and on top of that it was a relatively simpler model in terms of variables included. This can be validated by Bayesian Information Criterion (BIC), which adds a penalty for model complexity and over-fitting, and Model 2 had the lowest penalty added. Even though Model 4 had a lower RMSE, the difference was minimal, moreover, Model 2 had the lowest cross validated RMSE as well (11.41497). In conclusion, I preferred Model 2 for predicting earnings per hour for this specific occupation. 

From Model 1 to Model 4, below are the results for the 3 metrics used to evaluate the models.

**BIC** 23,671.8, 23,504.2, 23,560.1,23,685.0

**RMSE** 11.798, 11.386, 11.341, 11.305

**Cross-Validated RMSE** 11.80996, 11.41497, 11.42425, 11.45629



\newpage
# Appendix
## Box Plots for interaction terms
...
\newpage
## K-Fold Average RMSEs for all 4 Models
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, fig.height=8, fig.width=8}
Avgrmse
```
...
\newpage
## Prediction Results as per Model 2 
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, fig.height=8, fig.width=8}
pr
```

\newpage
## Cross Validated RMSE Detail
```{r echo=FALSE, fig.align='center', message=FALSE, warning=FALSE, fig.height=8, fig.width=8}
cv_mat
```
