---
title: "Technical_Report- Predictive Pricing Model"
author: "Rauhan Nazir"
date: "2/10/2022"
output:
  html_document:
...
u <- ggplot(rf_model_auto_var_imp_df[1:10,], aes(x=reorder(varname, imp), y=imp_percentage)) +
  geom_point(color='red', size=1) +
  geom_segment(aes(x=varname,xend=varname,y=0,yend=imp_percentage), color='red', size=0.75) +
  ylab("Importance (Percent)") +
  xlab("Variable Name") +
...
x <- ggplot(rf_model_auto_var_imp_grouped_df, aes(x=reorder(varname, imp), y=imp_percentage)) +
  geom_point(color='red', size=1) +
  geom_segment(aes(x=varname,xend=varname,y=0,yend=imp_percentage), color='red', size=0.7) +
  ylab("Importance (Percent)") +   xlab("Variable Name") +
  coord_flip() +
...
For the interaction terms boxplots of property types against price were plotted, conditioned on amenities and the neighbourhoods to see the impact it had on the prices of listings. After which the interaction terms between property type and amenities were created. The number of variables after this exercise was extremely large, and to restrict their number and include the ones that were actually important, we used LASSO. What LASSO did was that it assigned a value of 0 to the coefficients of the terms that were not important to the predictive model. Hence, helping out in the data engineering process. 
...
There were 4 models that were run with OLS, with each model getting more complex in terms of number of coefficients. Model 1 only had a single predictor, while model 4 had 330. These models were evaluated based on RMSE and BIC. Even though model 4 had the highest R-square, it also had the highest RMSEs and BIC, meaning that the model was being penalized for over-fitting. If we consider everything, model 3, with 146 coefficients, performed the best, as it had the lowest Test RMSE, and a relatively higher R-square compared to the first 2 models. All these regressions were run based on our training and test sets via 5-fold cross validations. 


## Random Forest

There were two types of Random Forests created. One where there were tuning parameters provided and the second was the one which had auto-tuning. It is a supervised learning algorithm where the end result is a combination of different trees, that were trained with the bagging method. So, the end result is essentially a result of averages of numerous trees, meaning that it is quite accurate of the actual representation. This was cross validated as well. The tuning parameters that we provided for the first one was as following:

-Number of variables (mtry) were from 8 to 12 
-Minimum number of observation (min. nodes) were 5,10,15

This model performed the best when mtry was equal to 12 and the min node size equals to 5, as this was the one with the lowest RMSE.
...
For both RF methods, variable importance graphs were constructed that showed which variables were the most important in predicting the prices. First these were done without any grouping, and during the second iteration some of the variables were grouped together, such as all the amenities’ dummies were grouped together. For the one where parameters were provided, the top three predictors were amenities, property type and reviews an apartment got. Same was the cases for the Auto-tuning RF. The difference between both RMSEs was minimal, however auto tuning one had a slightly lower one.
...
How CART with pruning works is that the training data is split randomly into several folds for instance 10, after which the pruning level is selected. 9 folds are used for creation of 9 pruned trees and an error is estimated on the last fold. The second step is repeated until all the pruning levels are used. The smallest error is found, and the pruning level assigned to it is used. Until pruning level is reached, all terminal nodes in the lowest tree level are removed and decision class is assigned to parent node. Decision value is equal to class with higher number of cases covered by node. Even though it’s RMSE was slightly higher than that of Random Forest, there is a high possibility of overfitting in the model. 

## GBM

Gradient boosting machine is a model which creates trees, and, with each iteration, it improves upon the previous tree. Hence, in principle this method is better than CART. In our predictive models, this was also the case in terms of the RMSEs of the models. This performed better than both the OLS and CART.

Below is the table the shows the RMSEs of all the models.


Based on these results, Random Forest with Auto Tuning was shortlisted, and based on this model Partial dependence plots were created, to provide insights and interpretable results

## Partial Dependence 

Based on the results from the variable importance plot we analyse the shape of association between average y and important x variables, condition on the rest (BEKES, 2022). So, it keeps everything else other than the predictive variable we want to see association of with the target variable. We created two such graphs, one with the property type and the second one with the number of people it accommodates. In the property type, Lofts were the ones which were priced the highest while for the number of people it accommodates, price went up when the persons accommodated increased. 
...
To further check our RF performance, we run sub sample on three x variables as shown in the figure below.
